...
export default function ProductForm() {
  ...
  const removeApp = (i) => set('applications', form.applications.filter((_, idx) => idx !== i))
  ...
  return (
    <div className="max-w-4xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Applications */}
        <div className="bg-white rounded shadow-sm p-6">
          <h2 className="font-bold text-dark mb-5 pb-3 border-b border-gray-100">適用場景</h2>
          <div className="flex gap-2 mb-4">
            <input value={appInput} onChange={e => setAppInput(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addApp())}
              placeholder="例：屋頂防水、浴室防水..."
              className="flex-1 border border-gray-200 px-4 py-2.5 text-sm focus:outline-none focus:border-primary rounded-sm" />
            <button type="button" onClick={addApp} className="bg-dark text-white px-4 py-2.5 text-sm rounded-sm hover:bg-gray-700">新增</button>
          </div>
          <div className="flex flex-wrap gap-2">
            {form.applications.map((a, i) => (
              <span key={i} className="bg-blue-50 text-blue-700 px-3 py-1 text-sm rounded flex items-center gap-2">
                {a}
                <button type="button" onClick={() => removeApp(i)} className="text-blue-400 hover:text-red-500">✕</button>
              </span>
            ))}
          </div>
        </div>
