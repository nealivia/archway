...
export default function ProductCard({ product, variant }) {
  ...
  // ── 預設 variant ──
  return (
    <div className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-gray-200 hover:shadow-xl transition-all duration-300 flex flex-col relative">
      {/* Compare toggle */}
      <button
      ...
      <Link to={`/products/${product.id}`} className="flex flex-col flex-1">
        ...
        <div className="px-4 py-3">
          {product.category_name && (
            <div className="text-[10px] text-gray-400 mb-1 uppercase tracking-widest">{product.category_name}</div>
          )}
          <h3 className="font-medium text-dark text-sm leading-snug tracking-tight">{product.name}</h3>
        </div>
      </Link>
    </div>
  )
